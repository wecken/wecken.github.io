export const JA_JP = {
  translation: {
    ...
    mbux_article_paragraph_03:
      "先代では、都道府県から番地まで順番に選んでいく住所入力方式を主な目的地入力方法として提供していました。これは日本独自のものでした。しかし、この方法では平均的な住所を入力するのに約90秒もかかっていたのです。そこで、住所、施設名、施設カテゴリ、電話番号、郵便番号など、さまざまなキーワードに対応した目的地を、ソフトウェアキーボードですぐに入力できるようフローを再設計しました。新しいコンセプトでは、同じ住所を入力するのに10秒しかかからず、前世代の9倍ものスピードで完了しました。また、音声入力も可能なため、運転中の住所入力もより安全かつスピーディーに行えます。",
    ...
    "Integrated one of the biggest online grocery store by AEON into Kurashiru, recipe video app, and designed the user experience completely from scratch":
      "レシピ動画アプリ「クラシル」に、イオンが運営する日本最大規模の食品スーパーを統合し、ユーザー体験をゼロからデザイン。",
    "Get delivered from your favorite local supermarket within 30 minuts":
      "お近くのスーパーマーケットから30分以内に配達",
    short_descripton_netsuper_01:
      "イオンが提供するネットスーパー機能をクラシルアプリ内に組み込むことで、クラシルを利用される数百万人以上のユーザーがレシピ検索から、食材購入までワンストップで行えるようにしました。",
    netsuper_article_paragraph_01:
      "ネットスーパーならではの課題としては、まずアイテム数が挙げられます。私たちのパートナーのオンラインストアには、最大で4万点のアイテムが掲載されています。また、Amazonのような通常のECと比較して、1回の買い物でカートに入れる商品数が非常に多いことも特徴です。通常のECでは、ユーザーは1〜2点の商品をまとめて購入しますが、ネットスーパーECでは20〜40点の商品を1つのカートで購入することが一般的です。つまり、商品を探して、カートに追加するまでの操作が効率的であることが非常に重要になります。",
    netsuper_article_paragraph_02:
      "この課題に対する私のアプローチは、カテゴリーナビゲーションのエクスペリエンスを注意深くデザインすることでした。スーパーマーケットで必要なものを探している自分を想像すると、それらがどこにあるのか、頭の中で位置を確認するはずです。あるいは、買いたいものを思い出したり、インスピレーションを得たりするために、コーナーを辿るように順番に歩いていくことになるでしょう。",
    netsuper_article_paragraph_03:
      "実際のスーパーマーケットで商品を見ているのと同じような体験ができるように、カテゴリー分けや画面に表示される順番をデザインしました。",
    netsuper_article_paragraph_04:
      "また、「Kurashiru」アプリでレシピを検索したり、ECで食材を検索して購入したり、自宅に配送してもらうことができるのも、このプロジェクトの面白い点です。",
    netsuper_article_paragraph_05:
      "このユニークなユースケースのUIをデザインしました。スーパーで定期的に購入する商品のセレクトは人それぞれ異なるため、このアプリでは食材ごとに複数の結果を提案しています。",
    short_descripton_kurashiru_delivery_01:
      "クラシルデリバリーは、いつものスーパーから食料品を配達してくれる、単独のネイティブアプリでした。React Nativeの技術により、シングルソースでiOSとAndroidに対応しました。ピーコックやイオンスタイルなどのフランチャイズスーパーと提携し、東京都港区、品川区、渋谷区、中央区の一部に対応しました。",
    kurashiru_delivery_article_paragraph_01:
      "クラシルデリバリーに先立って立ち上げたクラシルネットスーパープロジェクトでは、私たちのアプリはECフロントエンドのみを提供していました。配送やカスタマーサポートなど、それ以外のサービスは完全にパートナー企業側で行っていました。",
